...
//        Log.i(TAG, "来自服务器的消息 ====》" + byteBuf);

        try {
            byteBuf = byteBuf.replaceAll("转换消息为实体异常!your msg is:", "");
            JSONObject json = new JSONObject(byteBuf);
            if (json.getInt("ret") == NettyBaseInfo.CDM.heat) {
//                LogUtils.d(TAG, "recever heat >>>>>> " + NettyBaseInfo.CDM.heat);
...
//                if ("success".equals(json.getString("msg"))) {
                ...
                LogUtils.d(TAG, "register success");
//                }else{
...
//                }
            ...
            if (json.getInt("ret") == NettyBaseInfo.CDM.resetapp) {
                MessageInfo info = new MessageInfo();
                info.setCmd(RequestCmd.RESET_APP);
                LogUtils.d(TAG, "message = " + info.toString());
...
//                    LogUtils.i(TAG, "客户端：发送心跳");
                } else {
                    LogUtils.d(TAG, "客户端：发送心跳超时");
                    NettyClient.getInstance().setConnectStatus(false);
                    ctx.channel().close();
//                    NettyClient.getInstance().reconnect();
                }
                ...
                /*总超时*/
                LogUtils.i(TAG, "===客户端===(ALL_IDLE 总超时)");
                ctx.channel().close();
            }
        }
    }
